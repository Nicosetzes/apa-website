...
confirmCountriesForm.addEventListener("submit", (e) => {
  ...
  const confirmedCountries = cleanArrayFromValues.filter(
    (input) => input.value
  ); // Me quedo solo con los boxes chequeados
...
const inyectHTML = (param, countryCode) => {
  ...
  htmlNode = `<input name="ID${param.id}" class="form-check-input form-check-input-teams" type="checkbox" value="${param.id}|${countryCode}" id="ID${param.id}">
...
confirmDataForDbForm.addEventListener("submit", (e) => {
  ...
  const tournamentNameValue =
    confirmDataForDbForm.querySelector("#tournamentName").value;
  ...
  const humanPlayerCheckboxes = confirmDataForDbForm.querySelector(
    "#humanPlayerCheckboxes"
  );
  const teamsFromJSON = confirmDataForDbForm.querySelector("#teamsFromJSON");

  const amountOfPlayers = humanPlayerCheckboxes.querySelectorAll(
    'input[type="checkbox"]:checked'
  ).length;
  const amountOfTeams = teamsFromJSON.querySelectorAll(
    'input[type="checkbox"]:checked'
  ).length;
...
function atLeastOneCheckboxIsChecked(section) {
  const checkboxes = Array.from(
    document.querySelectorAll(`.form-check-input-${section}`)
  );
